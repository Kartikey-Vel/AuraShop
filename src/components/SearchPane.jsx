...
function SearchPaneImpl({ onAddToList, speakOut, onStatus }, ref) {
  ...
  return (
    <div className="w-full rounded-xl border border-indigo-100 bg-white p-3 dark:bg-slate-800 dark:border-fuchsia-700">
      <h2 className="mb-3 text-base sm:text-lg font-medium text-indigo-800 dark:text-fuchsia-300">
        Search products
      </h2>
      ...
      {Boolean(lastQuery || Object.keys(activeFilters || {}).length) && (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {lastQuery && (
            <span className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700 dark:bg-fuchsia-700/40 dark:text-fuchsia-200">
              Query: {lastQuery}
            </span>
          )}
          {activeFilters?.brand && (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full bg-indigo-100 px-2.5 py-1 text-xs font-medium text-indigo-800 hover:bg-indigo-200 dark:bg-fuchsia-700/60 dark:text-fuchsia-100"
